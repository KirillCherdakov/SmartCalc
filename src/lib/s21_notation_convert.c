...
int oper_priority(char *str, reg_templates template) {
  if (str[0] == '-' && str[1] == 'n') {
    ...
    return 2;
             ...
             !regexec(&template.mod, str, 0, NULL, 0)) {
    return 3;
  } else if (str[0] == '+' || str[0] == '-') {
    return 4;
  }
...
int oper_process(char *temp, char *dst, stack_s *stack,
...
int sort_station(char *src, char *dst, reg_templates template) {
  ...
  while (temp[0]) {
    if (temp[0] == ' ') {  // если в начале строки пробел, пропускаем его
      ...
      if (!need_number) {  // ожидался оператор, берем за таковой первый + или -
        ...
        negative = 0;
               ...
               (temp[0] == 'x')) {  // если в начале строки число
      ...
      if (need_number) {
        if (temp[0] == '-' && !negative) {  // ожидалось число, встречен первый
                                            // минус - отрицательное выражение
          negative = 1;
          buf[0] = '\0';
          strcat(buf, "-n");
          push_s(&stack, buf);
        ...
        } else {  // ожидалось число, встречен не первый минус или другой
                  // оператор - ошибка
          return BAD_INPUT;
        }
    ...
    } else if ((i = is_func(temp, template)) !=
               0) {  // если в начале строки функция
